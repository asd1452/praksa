...
class Korisnici extends Component {
  ...
  componentDidMount() {
    axios.get(`http://localhost:8000/api/korisnik`)
      .then(res => {
  ...
  render() {
    return (
      <div >
        <Container><h3 style={{marginLeft: '2em'}}>Korisnici usluga <Link to ='/portfolio' style={{ textDecoration: 'none', color: 'blue'}}>
 ...
 <tbody>
   <tr>
     ...
     <td>{ this.state.persons.map(person => <p>{person.tip}</p>)}</td>
   </tr>
 </tbody>
</table>
     </div>
